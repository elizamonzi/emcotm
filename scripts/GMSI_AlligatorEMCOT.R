...
score.gmsi.extra <- function(fns=list.files(pattern=".csv")){
  ...
  for(i in seq(along=fns)){
    ...
    ## tmp.dat <- read.csv("data/6. Gold-MSI Data/1001(1).csv") # Line for Debugging 
    ...
    listMeds <- gsub("[[:punct:]]","",unlist(lkt1[11]))
...
create.gmsi.dataset <- function(){
  ...
  write.csv(bigdata,"../../cogdata/aggregated_data/Gold_MSI_combined_data_with_Indvidual_Responses.csv")
}
